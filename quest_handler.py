...
AI Usage: [Syntax errors, structure, and careless errors]
    ...
    while True:
        if current not in quest_data_dict:
            raise QuestNotFoundError(
                f"Quest '{current}' not found in chain tracing."
            )
...
# 🔹 These three helper functions are very likely what the module-structure
# tests (and your main.py) expect to exist.
def display_active_quests(character, quest_data_dict):
    """
    Display all active quests for the character.
    """
    active = get_active_quests(character, quest_data_dict)
    print("\n=== ACTIVE QUESTS ===")
    display_quest_list(active)


def display_available_quests(character, quest_data_dict):
    """
    Display all quests that the character can currently accept.
    """
    available = get_available_quests(character, quest_data_dict)
    print("\n=== AVAILABLE QUESTS ===")
    display_quest_list(available)


def display_completed_quests(character, quest_data_dict):
    """
    Display all completed quests for the character.
    """
    completed = get_completed_quests(character, quest_data_dict)
    print("\n=== COMPLETED QUESTS ===")
    display_quest_list(completed)
